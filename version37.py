...
class Table(object):
    def __init__(self):
        ...
        self.b1 = tk.Button(self.root, text='Add', width=10,
    ...
    def window_table(self):
        ...
        mycol = mydb["data1"]
        last_all = list(mycol.find({}, {'_id': 0, 'ID': -1}).sort("ID", -1).limit(1))
        primary_number = last_all[0].get('ID')
        print(primary_number)
        self.primary_key = primary_number
    ...
    def add_data(self):
        ...
        self.my_str.set("Sensor saved !")
        self.t1.focus()
        showinfo(title='Information', message="Don't forget to save your data !")

        self.l5.after(3000, lambda: self.my_str.set(''))

        self.labels.append(self.t1.get("1.0", 'end-1c'))
    ...
    def delete_data(self):
        ...
        item_value = self.select_item()[1]
        print("ITEM VALUE", item_value)

        mycol.delete_one({"Sensor No": '{}'.format(item_value)})
    ...
    def save_data(self):
        ...
        many_data = self.convert_data()
        mycol.insert_many(many_data)
        showinfo(title='Information', message="Sensor Added !")
